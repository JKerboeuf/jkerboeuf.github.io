...
- Extraire le contenu du ficher .zip .zip précédemment téléchargé et exécutez `mre.sh` avec le chemin vers votre **.minecraft** ou vers un **fichier .jar**, vous pouvez également ajouter un chemin vers votre binaire Java `jar` (le chemin doit être vers le fichier binaire lui-même, pas vers le répertoire dans lequel il se trouve).
- Suivez les instructions données et extrayez ce que vous voulez !
...
```Shell
./mre.sh "/mnt/c/users/VOTRENOM/AppData/Roaming/.minecraft" "/chemin/vers/votre/jar"
```
